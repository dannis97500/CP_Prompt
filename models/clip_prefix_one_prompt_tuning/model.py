...
class ResidualAttentionBlock(nn.Module):
    ...
    def attention(self, x: torch.Tensor, prefix_prompt=None):
        ...
        if prefix_prompt is not None:
            if isinstance(prefix_prompt, list):
                pk = torch.cat([x[:1,:,:], prefix_prompt[0], x[1:,:,:]], dim=0)
                pv = torch.cat([x[:1,:,:], prefix_prompt[1], x[1:,:,:]], dim=0)
                return self.attn(x,pk, pv, need_weights=False, attn_mask=self.attn_mask)[0]
            else:
    ...
    def forward(self, x: torch.Tensor,prefix_prompt=None,ln_pre=None):
        ...
        if prefix_prompt is not None:
            if isinstance(prefix_prompt, list):# k neq v
                pk, pv = prefix_prompt
                pk=pk.to(x.dtype).permute(1, 0, 2)
                pv=pv.to(x.dtype).permute(1, 0, 2)
                pk=ln_pre(pk)
                pv=ln_pre(pv)
                pk=self.ln_1(pk)
                pv=self.ln_1(pv)
...
class Transformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor,prefix_prompt=None, task_id=None,ln_pre=None):
        C, B, D = x.shape

        for i, blk in enumerate(self.resblocks):
            if prefix_prompt is not None:
                p_list = prefix_prompt.forward(i,B, task_id=task_id)
            else:
                p_list = None 
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, instance_tokens=None,prefix_prompt=None, task_id=None):
        ...
        x = self.transformer(x,prefix_prompt=prefix_prompt, task_id=task_id,ln_pre = self.ln_pre)
