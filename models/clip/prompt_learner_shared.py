...
class PromptLearnerShared(nn.Module):
    def __init__(self, cfg, classnames, clip_model,shared_ctx_len):
        super().__init__()
        ...
        if ctx_init:
            ...
            prompt_prefix = " ".join(["X"] * (n_ctx+shared_ctx_len))
        ...
        self.register_buffer("token_suffix", embedding[:, 1 + n_ctx+shared_ctx_len:, :])  
    ...
    def forward(self,shared_ctx):
        ctx = self.ctx
        if ctx.dim() == 2:
            ctx = ctx.unsqueeze(0).expand(self.n_cls, -1, -1)
        if shared_ctx.dim() == 2:
            shared_ctx = shared_ctx.unsqueeze(0).expand(self.n_cls, -1, -1)
        ...
        if self.class_token_position == "end":
            prompts = torch.cat(
                [
                    prefix,  # (n_cls, 1, dim)
                    ctx,  # (n_cls, n_ctx, dim)
                    shared_ctx,# (n_cls, shared_ctx_len, dim)
                    suffix,  # (n_cls, *, dim)
                ],
                dim=1,
            )
