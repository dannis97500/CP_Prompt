...
# from .vittest import Attention,LayerScale,DropPath
...
class Attention(nn.Module):
    ...
    def forward(self, x,register_hook=False, prefix_prompt=None):
        B, N, C = x.shape
        ...
        if prefix_prompt is not None:
            pk, pv = prefix_prompt #prompt拆分
            pk = pk.reshape(B, -1, self.num_heads, C //
                            self.num_heads).permute(0, 2, 1, 3)
            pv = pv.reshape(B, -1, self.num_heads, C //
                            self.num_heads).permute(0, 2, 1, 3)
            k = torch.cat((pk, k), dim=2)#和原来的k拼接
            v = torch.cat((pv, v), dim=2)#和原来的v拼接
            
        attn = (q @ k.transpose(-2, -1)) * self.scale
        attn = attn.softmax(dim=-1)
        attn = self.attn_drop(attn)
        #如果register_hook参数为True,则将注意力矩阵的梯度保存到self.attn_gradients中,并注册一个钩子函数,用于在反向传播时计算梯度。
        if register_hook:
...
class Block(nn.Module):
    ...
    #传递prompt到注意力块
    def forward(self, x,register_hook=False, prefix_prompt=None):
        x = x + self.drop_path1(self.ls1(self.attn(self.norm1(x),register_hook=register_hook, prefix_prompt=prefix_prompt)))
...
class VisionTransformerPrefix(nn.Module):
    """ Vision Transformer
    ...
    def forward_features(self, x):
        ...
        x = self.pos_drop(x + self.pos_embed)#加了位置向量、随机dropout
        #直接输入到12个层中，需要拆解
...
@torch.no_grad()
def _load_weights(model: VisionTransformerPrefix, checkpoint_path: str, prefix: str = ''):
...
def _create_vision_transformer(variant, pretrained=False, **kwargs):
    ...
    model = build_model_with_cfg(
        VisionTransformerPrefix, variant, pretrained,
...
def vit_base_patch16_18x2_224(pretrained=False, **kwargs):
    ...
    return model
class ParallelBlock2(nn.Module):
...
if __name__ == '__main__':
    ...
    model=ParallelBlock2(768,6)
    model._forward_jit(1)
